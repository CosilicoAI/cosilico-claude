...
h/1.rac (main calculation) needs:
  ...
  - parameters.yaml: rate thresholds
...
h/2.rac (net capital gain) needs:
  - 26/1222.rac: net_long_term_capital_gain (EXTERNAL)
  - 26/1222.rac: net_short_term_capital_loss (EXTERNAL)
```

### 4. Identify External Dependencies

List variables needed from OTHER sections:
...
Based on dependencies, specify order:

```markdown
## Encoding Order

1. parameters.yaml (no dependencies)
2. h/2.rac (defines net_capital_gain - needed by others)
3. h/4.rac (28% rate gain)
4. h/5.rac (collectibles)
5. h/6.rac (unrecaptured 1250)
6. h/11.rac (qualified dividends)
7. h/3.rac (adjusted NCG - depends on 4, 6, 11)
8. h/1.rac (main calc - depends on all above)
9. UPDATE parent 26/1.rac to import from h/
```
...
Your analysis should produce a structured plan:

```markdown
# Encoding Plan: 26 USC § X
